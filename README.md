...
2. As a team, work through parts of the tutorial (listed in 3. below). For each part, your team should do the following:
   ...
   5. Ask everyone in the group if they follow what has happened in that section. Do not go on until each person states that they understand. If some team members do not understand, others should help explain the topics. If all of you are stuck, or need help, ask a member of the teaching team.
...
Write the Python code to plot two parabolas as lines on the same plot for the domain $$-2≤x≤2$$ for $$f=2$$ and $$f=6$$. Choose different colors for each line. Make the linewidth for $$f=6$$ a width of 6 and the linewidth for $$f=2$$ a width of 2.
...
Type `quit()` to return to normal. Alternatively, you can type the code into a file and add print statements after every line to see the output.
